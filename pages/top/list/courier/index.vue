    ...
    >
      <template #top>
        <v-toolbar style="cursor:auto" flat>
          <v-toolbar-title>برترین سفیران</v-toolbar-title>
          <!--           <v-select
            dense
            outlined
            hide-details
      ...
      <template #[`item.num`]="{ item }">
        <span> {{ topCouriers.indexOf(item) + 1 }}</span>
      </template>
...
export default class index extends Vue {
  ...
  private async getTopCouriers() {
    try {
      this.loading = true;
      await this.$axios
        .$get(`/admins/best-couriers/`, {
          params: {
        ...
        .then((response: TopCouriersModel) => {
          this.topCouriers = this.topCouriers.concat(response);
        });
    } catch (e : any) {
